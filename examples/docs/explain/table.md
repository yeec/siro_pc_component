...
:::demo 当`com-table`元素中注入`data`对象数组后，在`com-table-column`中用`prop`属性来对应对象中的键名即可填入数据，用`label`属性来定义表格的列名。可以使用`width`属性来定义列宽。
```html
  <template>
    <com-table
      :data="tableData"
...
<style>
  .com-table .warning-row {
    background: oldlace;
  }

  .com-table .success-row {
    background: #f0f9eb;
  }
</style>
...
:::demo 只要在`com-table`元素中定义了`height`属性，即可实现固定表头的表格，而不需要额外的代码。
```html
<template>
  <com-table
    :data="tableData"
...
:::demo 只需要在 com-table-column 里面嵌套 com-table-column，就可以实现多级表头。
```html
<template>
  <com-table
    :data="tableData"
    ...
    <com-table-column label="配送信息">
      <com-table-column
        prop="name"
        label="姓名"
        width="120">
      </com-table-column>
      <com-table-column label="地址">
        <com-table-column
          prop="province"
          label="省份"
          width="120">
...
:::demo Table 组件提供了单选的支持，只需要配置`highlight-current-row`属性即可实现单选。之后由`current-change`事件来管理选中时触发的事件，它会传入`currentRow`，`oldCurrentRow`。如果需要显示索引，可以增加一列`com-table-column`，设置`type`属性为`index`即可显示从 1 开始的索引号。
```html
<template>
  <com-table
    ref="singleTable"
...
:::demo 实现多选非常简单: 手动添加一个`com-table-column`，设`type`属性为`selection`即可；默认情况下若内容过多会折行显示，若需要单行显示可以使用`show-overflow-tooltip`属性，它接受一个`Boolean`，为`true`时多余的内容会在 hover 时以 tooltip 的形式显示出来。
```html
<template>
  <com-table
    ref="multipleTable"
...
:::demo 通过设置 type="expand" 和 `Scoped slot` 可以开启展开行功能，`com-table-column` 的模板会被渲染成为展开行的内容，展开行可访问的属性与使用自定义列模板时的 `Scoped slot` 相同。
```html
<template>
  <com-table
    :data="tableData"
...
| resizable | 对应列是否可以通过拖动改变宽度（需要在 com-table 上设置 border 属性为真） | boolean | — | true |
